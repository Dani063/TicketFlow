    ...
    for row in data.get("centers", []):
        ...
        for cat_row in row["categories"]:
            category, _ = HelpCategory.objects.update_or_create(
                ...
                defaults={
                    "source_id": cat_row.get("source_id"),
                    "translation_key": cat_row.get("translation_key", ""),
            ...
            for sec_row in cat_row["sections"]:
                section, _ = HelpSection.objects.update_or_create(
                    ...
                    defaults={
                        "source_id": sec_row.get("source_id"),
                        "translation_key": sec_row.get("translation_key", ""),
                ...
                for art_row in sec_row["articles"]:
                    ...
                    HelpArticle.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "translation_key": art_row.get("translation_key", ""),
                            "title": art_row["title"], "body_html": body,
                            "body_text": html.unescape(strip_tags(body)),
                            "promoted": art_row.get("promoted", False),
                            "position": art_row.get("position", 0),
                            "source_url": art_row.get("source_url", ""),
                            "source_updated_at": parse_datetime(art_row["source_updated_at"])
                            if art_row.get("source_updated_at") else None,
                            "published": True,
                        },
                    )
...
class Migration(migrations.Migration):
    dependencies = [("app", "0051_seed_help_center_content")]
    operations = [migrations.RunPython(refresh_help_centers, migrations.RunPython.noop)]
